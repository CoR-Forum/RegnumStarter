...
void MemoryManipulation(const std::string& option, float newValue) {
    ...
    for (const auto& pointer : pointers) {
        if (pointer.name == option) {
            ...
            SIZE_T bytesRead;
            LogDebug("Found " + option + " pointer address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(optionPointer)));
            ...
            if (pointer.offsets.empty()) {
                ...
                LogDebug("Writing directly to " + option + " final pointer address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(finalAddress)));
            } else {
                for (size_t i = 0; i < pointer.offsets.size(); ++i) {
                    if (ReadProcessMemory(hProcess, (LPCVOID)finalAddress, &finalAddress, sizeof(finalAddress), &bytesRead)) {
                        ...
                        LogDebug("Reading " + option + " pointer address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(finalAddress)));
                    } else {
                        LogDebug("Failed to read " + option + " pointer from memory. Error code: " + std::to_string(GetLastError()) + " Address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(finalAddress)));
            ...
            if (WriteProcessMemory(hProcess, (LPVOID)finalAddress, &newValue, sizeof(newValue), NULL)) {
                ...
                LogDebug("Failed to write new " + option + " value. Error code: " + std::to_string(GetLastError()) + " Address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(finalAddress)) + " Value: " + std::to_string(newValue) + " Size: " + std::to_string(sizeof(newValue)));
                MessageBox(NULL, ("Failed to write new " + option + " value. Error code: " + std::to_string(GetLastError()) + " Address: " + "0x" + std::to_string(reinterpret_cast<uintptr_t>(finalAddress)) + " Value: " + std::to_string(newValue) + " Size: " + std::to_string(sizeof(newValue))).c_str(), "Error", MB_ICONERROR | MB_TOPMOST);
            }
        }
    }
